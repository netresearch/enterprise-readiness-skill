...
**A refactor mints new issue keys, and the mark does not follow them.** The mark binds to the issue key. Tracking across analyses survives shifted lines, but not flagged lines moved into a new function or scope: the next analysis raises the same finding under new keys, the old accepted keys stay resolved on locations that no longer exist, and the gate goes red again. On a PR that both refactors and carries intentional findings, land the final code shape first, then mark whatever the fresh analysis reports. Re-query `api/issues/search?...&pullRequest=<n>&resolved=false` after every push that touches the flagged files — a gate that passed can re-redden without any new finding.
