...
| `dependency_monitoring` | A CI audit that fails on a known vulnerability (Composer Audit, govulncheck), or Renovate with vulnerability alerts. The criterion also asks for fixing, which a failing audit enforces; a Dependabot file that only schedules updates shows neither. Describe what the CI enforces, including its exceptions: the shared Composer Audit passes with a warning when the advisory source stays unreachable, and govulncheck fails only on reachable vulnerabilities, in strict mode (the default) |
