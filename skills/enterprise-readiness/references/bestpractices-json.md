...
| `test` | Test files in the tree, and a pull-request workflow that invokes them (`go test`, `bin/phpunit`, `composer test:php:unit`, or a shared CI reusable with tests on by default) — the criterion accepts a CI script as the documented way to run the suite, and the licence makes the suite FLOSS. Cite the test directory for the count, and leave out `Tests/Architecture/` — phpat rules there run under PHPStan, not PHPUnit |
...
| `dependency_monitoring` | A CI audit that fails on a known vulnerability (Composer Audit, govulncheck). The criterion also asks for fixing, which a failing audit enforces. Renovate counts where it actually runs in the repository (it has opened pull requests there) and GitHub's vulnerability alerts are enabled: `vulnerabilityAlerts` opens a fix pull request labelled `security`, and because an alert forces only its own keys over the package rules, the shared config's automerge for patch and minor updates applies to it — a major fix waits for review. Say so in the justification. A Dependabot file that only schedules updates shows neither. Describe what the CI enforces, including its exceptions: the shared Composer Audit passes with a warning when the advisory source stays unreachable, and govulncheck fails only on reachable vulnerabilities, in strict mode (the default) |
